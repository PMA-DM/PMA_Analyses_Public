...
*
* This .do file will only work on Phase 3 CEI cross sectional datasets. You 
...
/*******************************************************************************
*
*  FILENAME:		PMA_CEI_Phase3XS_ResultsBrief.do
*  PURPOSE:			Generate the .xls output for the PMA Phase 1 XS Results Brief
*  CREATED BY: 		Guy Bai (gbai5@jhu.edu)
*  DATA IN:			PMA's Phase3 XS CEI publicly released dataset
*  DATA OUT: 		PMA_COUNTRY_Phase3_XS_CEI_Analysis_DATE.dta
*  FILE OUT: 		PMA_COUNTRY_Phase3_XS_CEI_Analysis_DATE.xls
*  LOG FILE OUT: 	PMA_COUNTRY_Phase3_XS_CEI_Log_DATE.log
*
*******************************************************************************/
...
local datadir "~/Dropbox (Gates Institute)/UG-Uganda/PMAUG_Datasets/Phase3/Prelim100/UGP3_CQ_NONAME_22Feb2023.dta"
...
if check!=1 {
	di in smcl as error "The dataset you are using is not a PMA phase 1 XS dataset. This .do file is to generate the .xls files for PMA Phase 3 XS surveys only. Please use a PMA Phase 3 XS survey and rerun the .do file"
	exit
	}
	drop check
...
* Start log file
log using "`briefdir'/PMA_`country'_Phase3_XS_CEI_Log_`date'.log", replace		

* Set local for xls file
local tabout "PMA_`country'_Phase3_XS_CEI_Analysis_`date'.xls"

* Set local for dataset
local dataset "PMA_`country'_Phase3_XS_CEI_Analysis_`date'.dta"
