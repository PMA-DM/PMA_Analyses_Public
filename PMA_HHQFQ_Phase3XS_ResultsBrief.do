...
* 	available Phase 3 SDP, CQ and Panel datasets and other surveys in the  
...
/*******************************************************************************
*
*  FILENAME:		PMA_HHQFQ_Phase3XS_ResultsBrief.do
*  PURPOSE:			Generate the .xls output for the PMA Phase 3 XS Results Brief
*  CREATED BY: 		Guy Bai (gbai5@jhu.edu)
*  DATA IN:			PMA's Phase3 XS HHQFQ publicly released dataset
*  DATA OUT: 		PMA_COUNTRY_PHASE3_XS_HHQFQ_Analysis_DATE.dta
*  FILE OUT: 		PMA_COUNTRY_PHASE3_XS_HHQFQ_Analysis_DATE.xls
*  LOG FILE OUT: 	PMA_COUNTRY_PHASE3_XS_HHQFQ_Log_DATE.log
*
*******************************************************************************/
...
local datadir "~/Dropbox (Gates Institute)/BF-Burkina/PMABF_Datasets/Phase3/Final_PublicRelease/HQFQ/PMA2022_BFP3_HQFQ_v2.0_12Jul2023/PMA2022_BFP3_HQFQ_v2.0_12Jul2023.dta"
...
*		analyzing the data. Generally, all Phase 3 data contains both 
...
* Confirm that it is phase 3 data
if country=="Burkina" {
	gen check=(phase==3)
	}
else if country=="DRC" {
...
if check!=1 {
	di in smcl as error "The dataset you are using is not a PMA phase 3 XS dataset. This .do file is to generate the .xls files for PMA Phase 3 XS surveys only. Please use a PMA Phase 3 XS survey and rerun the .do file"
	exit
	}
	drop check
...
* Start log file
log using "`briefdir'/PMA_`country'_Phase3_XS_HHQFQ_Log_`date'.log", replace		

* Set local for xls file
local tabout "PMA_`country'_Phase3_XS_HHQFQ_Analysis_`date'.xls"

* Set local for dataset
local dataset "PMA_`country'_Phase3_XS_HHQFQ_Analysis_`date'.dta"
...
* Update indicators labels to match labels in Phase 3 briefs
