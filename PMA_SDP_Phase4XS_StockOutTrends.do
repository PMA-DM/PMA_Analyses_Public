...
* 	generate its Phase4 cross sectional results briefs using PMA's publicly  
* 	available SDP dataset
*
* This .do file will only work on Phase 4 SDP cross sectional datasets. You 
*   can  find the .do files to generate the .xls file outputs for PMA's publicly
* 	available Phase 4 HHQFQ and CQ datasets and other surveys in the  
...
/*******************************************************************************
*
*  FILENAME:		PMA_Phase4XS_StockOutTrends.do
*  PURPOSE:			Generate the .xls output with SDP trends for the PMA Phase 4 XS Results Brief
*  CREATED BY: 		Claire Silberg (csilber4@jhu.edu.edu)
*  DATA IN:			PMA's XS SDP publicly released datasets from PMA2020 rounds to PMA-Phases
*  DATA OUT: 		PMA_COUNTRY_Phase4_XS_StockOutTrends_DATE.dta
*  FILE OUT: 		PMA_COUNTRY_Phase4_XS_StockOutTrends_DATE.xls
*  LOG FILE OUT: 	PMA_COUNTRY_Phase4_XS_StockOutTrends_Log_DATE.log
*
*******************************************************************************/
...
*		- For example (PC):
* 		  local dataset1 "~\PMA2020\PMA2018_NGR5_National_HHQFQ_v5_4Nov2019.dta"
...
* Start log file
log using "`briefdir'/PMA_`country'_Phase4_XS_SDP_StockOutTrends_Log_$date.log", replace		

* Set local for xls file
local excel "PMA_`country'_Phase4_XS_SDP_StockOutTrends_$date.xls"

* Set local for dataset
local dataset "PMA_`country'_Phase4_XS_SDP_StockOutTrends_$date.dta"
